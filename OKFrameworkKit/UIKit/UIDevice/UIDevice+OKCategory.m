...
@implementation UIDevice (OKCategory)
...
- (OKHardware)ok_hardware {
    NSString *hardware = [self ok_hardwareString];
...
- (float)__hardwareNumber:(OKHardware)hardware {
    switch (hardware) {
...
- (BOOL)ok_isCurrentDeviceHardwareBetterThan:(OKHardware)hardware {
    float otherHardware   = [self __hardwareNumber:hardware];
    float currentHardware = [self __hardwareNumber:[self ok_hardware]];
    return currentHardware >= otherHardware;
}

- (CGSize)ok_backCameraStillImageResolutionInPixels {
    switch ([self ok_hardware]) {
    case IPHONE_2G:
    case IPHONE_3G:
    ...
    default:
        ...
        NSLog(@"Your device is: %@", [self ok_hardwareDescription]);
        break;
    }
    return CGSizeZero;
}
...
+ (NSUInteger)ok_getSysInfo:(uint)typeSpecifier {
...
+ (NSUInteger)ok_cpuFrequency {
    return [self ok_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)ok_busFrequency {
    return [self ok_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)ok_ramSize {
    return [self ok_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)ok_cpuNumber {
    return [self ok_getSysInfo:HW_NCPU];
}

+ (NSUInteger)ok_totalMemoryBytes {
    return [self ok_getSysInfo:HW_PHYSMEM];
}
